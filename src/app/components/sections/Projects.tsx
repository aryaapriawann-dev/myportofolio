...
export default function Projects({ data }: { data?: Project[] } = {}) {
  ...
  return (
    <section id="projects" className="space-y-10">
      ...
      {filteredProjects.length > 0 ? (
        <div className="grid gap-6 md:grid-cols-2">
          {filteredProjects.map((p) => (
            <div
              key={p.id ?? p.name}
              className="group flex flex-col overflow-hidden rounded-2xl border border-zinc-200/60 dark:border-zinc-800/60 bg-white/70 dark:bg-zinc-900/70 shadow-sm transition-all duration-300 hover:border-zinc-300 dark:hover:border-zinc-700 hover:shadow-md cursor-pointer"
            ...
            >
              ...
              <div className="flex flex-1 flex-col p-6 md:p-7">
                <h3 className="text-base font-bold text-neutral-900 dark:text-white group-hover:text-red-600 dark:group-hover:text-red-500 transition-colors">{p.name}</h3>
              ...
              {/* Action Buttons */}
              <div className="flex items-center justify-between border-t border-zinc-100 dark:border-zinc-800/60 bg-white/40 dark:bg-zinc-900/40 px-6 py-4" onClick={(e) => e.stopPropagation()}>
                ...
                <button
                  type="button"
                  onClick={() => setSelectedProject(p)}
                  className="font-label text-[10px] font-bold uppercase tracking-widest text-red-600 dark:text-red-400 hover:underline"
                >
                  Detail &rarr;
                </button>
              </div>
      ...
      {/* Case Study Modal */}
      {selectedProject && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={() => setSelectedProject(null)}>
          <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 md:p-8 shadow-2xl space-y-6" onClick={(e) => e.stopPropagation()}>
            <button
              onClick={() => setSelectedProject(null)}
              className="absolute right-4 top-4 rounded-full p-2 text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors"
            >
              ✕
            </button>

            {selectedProject.image && (
              <div className="relative aspect-video w-full overflow-hidden rounded-xl bg-zinc-100 dark:bg-zinc-950">
            ...
            <div>
              <span className="font-label text-[10px] font-bold uppercase tracking-widest text-red-600 dark:text-red-400">
            ...
            <div>
              <h4 className="text-xs font-bold uppercase tracking-wider text-neutral-400 dark:text-zinc-500 mb-2">Deskripsi Proyek</h4>
              <p className="text-sm leading-relaxed text-neutral-700 dark:text-zinc-300">{selectedProject.desc}</p>
            </div>

            <div>
              <h4 className="text-xs font-bold uppercase tracking-wider text-neutral-400 dark:text-zinc-500 mb-2">Teknologi & Stack</h4>
              <div className="flex flex-wrap gap-2">
                {selectedProject.tech.map((t) => (
                  <span key={t} className="rounded-md bg-neutral-100 dark:bg-zinc-800 px-3 py-1 text-xs font-medium text-neutral-700 dark:text-zinc-300">
                    {t}
                  </span>
                ))}
              </div>
            ...
            <div className="flex items-center gap-4 pt-4 border-t border-zinc-100 dark:border-zinc-800">
              {selectedProject.liveUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="rounded-lg bg-red-brand px-4 py-2 font-label text-[10px] font-bold uppercase tracking-widest text-white hover:bg-red-700 transition"
                >
                  Buka Live Demo
                </a>
              )}
              {selectedProject.githubUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="rounded-lg border border-zinc-300 dark:border-zinc-700 px-4 py-2 font-label text-[10px] font-bold uppercase tracking-widest text-neutral-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition"
                >
                  Lihat Repository
                </a>
              )}
